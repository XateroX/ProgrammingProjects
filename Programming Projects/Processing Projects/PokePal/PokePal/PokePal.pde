...
boolean typeMatchupMode;
int typeClicked;
boolean displayMatchupMode;
...
void setup()
{
    ...
    typeMatchupMode    = true;
    typeClicked        = -1;
    displayMatchupMode = false;
...
void typeMatchupMode()
{
    for (String type : typetoind.keys())
    {
        ...
        translate(0,height/(float)typetoind.size());
    }
}

String indtotype(int ind)
{
    if (ind == 0) {return "Normal";}
    if (ind == 1) {return "Fighting";}
    if (ind == 2) {return "Flying";}
    if (ind == 3) {return "Poison";}
    if (ind == 4) {return "Ground";}

    if (ind == 5) {return "Rock";}
    if (ind == 6) {return "Bug";}
    if (ind == 7) {return "Ghost";}
    if (ind == 8) {return "Steel";}
    if (ind == 9) {return "Fire";}

    if (ind == 10) {return "Water";}
    if (ind == 11) {return "Grass";}
    if (ind == 12) {return "Electric";}
    if (ind == 13) {return "Psychic";}
    if (ind == 14) {return "Ice";}

    if (ind == 15) {return "Dragon";}
    if (ind == 16) {return "Dark";}
    else {return null;}
}
...
void displayTypeCircle(String type)
{
    colorMode(RGB,255);
    pushMatrix();
    float sVal = height / (typetoind.size() * 234.0);
    scale(sVal);
    if (typeClicked == typetoind.get(type)) {fill(255*sin((float)frameCount/5.0)); noStroke(); rect(0,0,typeIcons.get(typetoind.get(type)).width*1.2, typeIcons.get(typetoind.get(type)).height);}
    fill(0,0);
    if (typeClicked != -1)
    {
        if (float(Pokedex.typechart.getStringColumn(type)[typeClicked]) == 2.0)
        {
            fill(255,0,0);
        }
        if (float(Pokedex.typechart.getStringColumn(type)[typeClicked]) == 0.5)
        {
            fill(100,200,100);
        }
        if (float(Pokedex.typechart.getStringColumn(indtotype(typeClicked))[typetoind.get(type)]) == 2.0)
        {
            colorMode(HSB, 255);
            fill(255*(1+sin(PI*(float)frameCount/60.0))/2,255,255, 255);
            //println(type, "(",typetoind.get(type), ")"," fighting ", typeClicked, " is ", float(Pokedex.typechart.getStringColumn(typeClicked)[typetoind.get(type)]));
        }
        if (float(Pokedex.typechart.getStringColumn(indtotype(typeClicked))[typetoind.get(type)]) == 0.0)
        {
            colorMode(RGB, 255);
            fill(255*(1+sin(PI*(float)frameCount/60.0))/2,255*(1+sin(PI*(float)frameCount/60.0))/2,0);
            //println(type, "(",typetoind.get(type), ")"," fighting ", typeClicked, " is ", float(Pokedex.typechart.getStringColumn(typeClicked)[typetoind.get(type)]));
        }
        if (float(Pokedex.typechart.getStringColumn(type)[typeClicked]) == 0.0)
        {
            colorMode(RGB, 255);
            fill(255*(1+sin(PI*(float)frameCount/30.0))/2, 0, 255*(1+sin(PI*(float)frameCount/30.0))/2);
            //println(type, "(",typetoind.get(type), ")"," fighting ", typeClicked, " is ", float(Pokedex.typechart.getStringColumn(typeClicked)[typetoind.get(type)]));
        }
        
        rect(0,0,typeIcons.get(typetoind.get(type)).width, typeIcons.get(typetoind.get(type)).height);
    }
    
    //{translate(sVal*234.0/1.5, sVal*234.0/2.0); fill(0,255 * (1+sin(PI * (float)frameCount/60.0))/2.0,0); ellipse(0,0,250,250); translate(-sVal*234.0/2.0, -sVal*234.0/2.0);}
...
void plot(int[] yvalues, float w, float h)
{
 ...
 void mousePressed()
 {
    if (typeMatchupMode)
    {
        typeClicked = floor(mouseY/(height/(float)typetoind.size()));
    }
 }
