...
void setup()
{
pdex= new pokedex("pokedex.csv");

String[] names = pdex.getPokemonNames();
Integer[] hp       = new Integer[names.length];
Integer[] defs     = new Integer[names.length];
Integer[] spdefs   = new Integer[names.length];

try
{
	PrintWriter writer = new PrintWriter("C:/Users/danlo/OneDrive/Desktop/Programming Projects/Processing Projects/PokePal_Analysis/hp_defs_spdefs.txt", "UTF-8");
	for (int i = 0; i < names.length; i++)
	{
		String name = names[i];
		//hp[i]     = Integer.parseInt(pdex.pokemonData(name).getString(3));
		//defs[i]   = Integer.parseInt(pdex.pokemonData(name).getString(5));
		//spdefs[i] = Integer.parseInt(pdex.pokemonData(name).getString(7));
		
		writer.println("hp[" + pdex.pokemonData(name).getString(3) + "]");
		if (i < 100) println("hp[" + pdex.pokemonData(name).getString(3) + "]");
		writer.println("defs[" + pdex.pokemonData(name).getString(5) + "]");
		if (i < 100) println("defs[" + pdex.pokemonData(name).getString(5) + "]");
		writer.println("spdefs[" + pdex.pokemonData(name).getString(7) + "]");
		if (i < 100) println("spdefs[" + pdex.pokemonData(name).getString(7) + "]");
		if (i < 100) println("wrote line", i);
	}
	writer.flush();
	writer.close();
	println("closed file");
...
int Damage(pokemon pa, pokemon pb, int power, boolean sp, String type)
{
	...
	if (sp)
	{
		...
		B = 2.0+A/50.0;
		C = B * 1.5 * pa.typematchup(pb, type, pdex);
		dmg = ceil(C);
	}
	if (!sp)
	{
...
class pokemon
{
	...
	float typematchup(pokemon pb, String atype, pokedex pdex)
	{
		int row_ind = 0;
		//println("Type: " + atype);
		for (TableRow row : pdex.typechart.rows())
		{
		...
		float multiplier                       = float(pdex.typechart.getStringColumn(pb.type1)[row_ind]);
		if (!pb.type2.equals("")) {multiplier *= float(pdex.typechart.getStringColumn(pb.type2)[row_ind]);}
		return multiplier;
	}
